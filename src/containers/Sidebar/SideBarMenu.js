...
const SideBarMenu = () => {
    ...
    const currentCompanyData = currentCompany !== null ? companies.filter((item) => item.id == currentCompany.id) : [] ;
    const channels = currentCompanyData.length ? currentCompanyData[0].channels: [];
    let channelMenu = [];
    
    if ( channels != undefined && channels.length > 0 ) { channels.map(channel => {
